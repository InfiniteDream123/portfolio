...
const targetText = 'JOSHUA MARK STROMME'
...
export default function SplashScreen({ onFinish }) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            const scramble = Array.from(targetText).map((char, i) =>
                i < frame ? targetText[i] : String.fromCharCode(65 + Math.floor(Math.random() * 26))
            ).join('')
    ...
    return (
        <div className="fixed inset-0 bg-black flex flex-col items-center justify-center z-50 text-white font-mono overflow-hidden crt">
            ...
            {/* Scan line effect */}
            <motion.div
                className="absolute top-0 left-0 w-full h-[2px] bg-green-400 shadow-md opacity-70 z-40"
                initial={{ y: 0 }}
                animate={{ y: '100%' }}
            ...
            >
                ...
                {/* Animated ID */}
                <div className="bg-gray-900 border border-green-400 px-6 py-3 mt-6 rounded text-2xl tracking-[0.3em] text-green-300 shadow-md">
                    {displayText}
                </div>
                ...
                {confirmed && (
                    <motion.div
                        ...
                        className="mt-4 text-green-400 text-lg tracking-wider"
                    >
                        ✅ Identity Confirmed
                    </motion.div>
                )}
